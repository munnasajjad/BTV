...
public partial class app_WorkFlowForMaintenance : System.Web.UI.Page
{
                ...
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    if (userDetails.Rows.Count > 0)
                    {
                        ...
                        mnId = userDetails.Rows[0]["WorkFlowTypeID"].ToString();
                    }
                    if (userPpriority > 0)
                    {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE [Priority]='" + nextUserPriority + "' AND WorkFlowTypeID='" + mnId + "' AND WFU.WorkFlowType='MN'");
                    string nextEmpName = "";
                    string nextUserId = "";
                    //string priorty = "0";
                    string nextTypeID = "";
                    ...
                    if (nextUserDetails.Rows.Count > 0)
                    {
                        ...
                        SQLQuery.ExecNonQry("UPDATE Maintenance SET CurrentWorkflowUser='" + nextEmpName + "' WHERE MaintenanceId = '" + mnId + "'");
                    }
                    string status = "";
                    string updateQuery = "";
                    if (nextUserDetails.Rows.Count == 0)
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update Maintenance SET  " + updateQuery + " WHERE MaintenanceId='" + mnId + "'");
                    BindWorkFlowUserGridView(mnId);
                    txtYourRemark.Text = "";
                    ReloadNotification();
                    PermissionToAction();
                }
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            try
            {
                ...
                if (status != "Hold")
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE  WorkFlowTypeID='" + lvId + "' AND WorkFlowType='MN'");
                    string empName = "";
                    string email = "";
                    string voucherNo = "";
                    ...
                    SQLQuery.ExecNonQry(" Update Maintenance SET WorkflowStatus='Hold',ReturnOrHoldUserID='" + wUserId + "',WorkflowApprovedDate= '" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss tt") + "' WHERE MaintenanceId='" + lvId + "'");
            ...
            if (status != "Return")
            {
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update Maintenance SET WorkflowStatus='Return', ReturnOrHoldUserID='" + wUserId + "' WHERE MaintenanceId='" + lvId + "'");
                    BindWorkFlowUserGridView(lvId);
                    txtYourRemark.Text = "";
                    PermissionToAction();
                    ReloadNotification();
    ...
    protected void btnDecline_OnClick(object sender, EventArgs e)
    {
        ...
        if (status != "Decline")
        {
            try
            {
                ...
                RunQuery.SQLQuery.ExecNonQry("Update Maintenance SET WorkflowStatus='Decline', ReturnOrHoldUserID='" + wUserId + "' WHERE MaintenanceId='" + lvId + "'");
                BindWorkFlowUserGridView(lvId);
                txtYourRemark.Text = "";
                PermissionToAction();
                ReloadNotification();
