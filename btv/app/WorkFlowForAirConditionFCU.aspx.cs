...
public partial class app_WorkFlowForAirConditionFCU : System.Web.UI.Page
{
    ...
    private void BindWorkFlowUserGridView(string id)
    {
        ...
        //string query = @"SELECT  Employee.Name AS EmployeeName ,WorkFlowUser.Priority, convert(varchar, WorkFlowUser.EsclationStartTime, 103) AS EsclationStartTime, convert(varchar, WorkFlowUser.EsclationEndTime, 103) AS EsclationEndTime,UserRemarks, ApproveDeclineDate, PermissionStatus FROM WorkFlowUser INNER JOIN Employee ON WorkFlowUser.EmployeeID = Employee.EmployeeID WHERE WorkFlowTypeID='" + acfId + "' Order By Priority DESC ";
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            ...
            if (statusPermission != "Approved")
            {
                try
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE [Priority]='" + nextUserPriority + "' AND WorkFlowTypeID='" + esId + "' AND WFU.WorkFlowType='ACF'");
                    string nextEmpName = "";
                    string nextUserId = "";
                    //string priorty = "0";
                    string nextTypeID = "";
                    ...
                    if (nextUserDetails.Rows.Count > 0)
                    {
                        ...
                        SQLQuery.ExecNonQry("UPDATE AirConditionFCU SET CurrentWorkflowUser='" + nextEmpName + "' WHERE AirConditionFcuId = '" + esId + "'");
                    }
                    string status = "";
                    string updateQuery = "";
                    if (nextUserDetails.Rows.Count == 0)
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update AirConditionFCU SET  " + updateQuery + " WHERE AirConditionFcuId='" + esId + "'");
    ...
    protected void btnHold_OnClick(object sender, EventArgs e)
    {
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            try
            {
                string id = Request.QueryString["Id"];
                string wUserId = EncryptDecrypt.DecryptString(id);
                string acfId = SQLQuery.ReturnString("SELECT WorkFlowTypeID From WorkFlowUser Where WorkFlowUserID='" + wUserId + "'");
                string status = SQLQuery.ReturnString("SELECT PermissionStatus From WorkFlowUser Where WorkFlowUserID='" + wUserId + "'");
                if (status != "Hold")
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE  WorkFlowTypeID='" + acfId + "' AND WorkFlowType='ACF'");
                    string empName = "";
                    string email = "";
                    string voucherNo = "";
                    ...
                    SQLQuery.ExecNonQry(" Update AirConditionFCU SET WorkflowStatus='Hold',ReturnOrHoldUserID='" + wUserId + "',WorkflowApprovedDate= '" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss tt") + "' WHERE AirConditionFcuId='" + acfId + "'");
                    BindWorkFlowUserGridView(acfId);
                    txtYourRemark.Text = "";
                ...
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    if (userDetails.Rows.Count > 0)
                    {
                        ...
                        acfId = userDetails.Rows[0]["WorkFlowTypeID"].ToString();
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update AirConditionFCU SET WorkflowStatus='Return', ReturnOrHoldUserID='" + wUserId + "' WHERE AirConditionFcuId='" + acfId + "'");
                    BindWorkFlowUserGridView(acfId);
                    txtYourRemark.Text = "";
                    PermissionToAction();
                    ReloadNotification();
    ...
    protected void btnDecline_OnClick(object sender, EventArgs e)
    {
        ...
        if (status != "Decline")
        {
            try
            {
                ...
                RunQuery.SQLQuery.ExecNonQry(" Update AirConditionFCU SET WorkflowStatus='Decline', ReturnOrHoldUserID='" + wUserId + "' WHERE AirConditionFcuId='" + acfId + "'");
                BindWorkFlowUserGridView(acfId);
                txtYourRemark.Text = "";
                PermissionToAction();
                ReloadNotification();
