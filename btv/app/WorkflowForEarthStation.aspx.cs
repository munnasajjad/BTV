...
public partial class app_WorkflowForEarthStation : System.Web.UI.Page
{
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            ...
            if (statusPermission != "Approved")
            {
                try
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE [Priority]='" + nextUserPriority + "' AND WorkFlowTypeID='" + esId + "' AND WFU.WorkFlowType='ES'");
                    string nextEmpName = "";
                    string nextUserId = "";
                    //string priorty = "0";
                    string nextTypeID = "";
                    ...
                    if (nextUserDetails.Rows.Count > 0)
                    {
                        ...
                        SQLQuery.ExecNonQry("UPDATE EarthStation SET CurrentWorkflowUser='" + nextEmpName + "' WHERE EarthStationId = '" + esId + "'");
                    }
                    string status = "";
                    string updateQuery = "";
                    if (nextUserDetails.Rows.Count == 0)
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update EarthStation SET  " + updateQuery + " WHERE EarthStationId='" + esId + "'");
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            try
            {
                ...
                if (status != "Hold")
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE  WorkFlowTypeID='" + lvId + "' AND WorkFlowType='ES'");
                    string empName = "";
                    string email = "";
                    string voucherNo = "";
                    ...
                    SQLQuery.ExecNonQry(" Update EarthStation SET WorkflowStatus='Hold',ReturnOrHoldUserID='" + wUserId + "',WorkflowApprovedDate= '" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss tt") + "' WHERE EarthStationId='" + lvId + "'");
            ...
            if (status != "Return")
            {
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update EarthStation SET WorkflowStatus='Return', ReturnOrHoldUserID='" + wUserId + "' WHERE EarthStationId='" + lvId + "'");
                    BindWorkFlowUserGridView(lvId);
                    txtYourRemark.Text = "";
                    PermissionToAction();
                    ReloadNotification();
    ...
    protected void btnDecline_OnClick(object sender, EventArgs e)
    {
        ...
        if (status != "Decline")
        {
            try
            {
                ...
                RunQuery.SQLQuery.ExecNonQry("Update EarthStation SET WorkflowStatus='Decline', ReturnOrHoldUserID='" + wUserId + "' WHERE EarthStationId='" + lvId + "'");
                BindWorkFlowUserGridView(lvId);
                txtYourRemark.Text = "";
                PermissionToAction();
                ReloadNotification();
