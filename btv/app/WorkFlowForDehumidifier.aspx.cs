...
public partial class app_WorkFlowForDehumidifier : System.Web.UI.Page
{
                ...
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    if (userDetails.Rows.Count > 0)
                    {
                        ...
                        dmId = userDetails.Rows[0]["WorkFlowTypeID"].ToString();
                    }
                    if (userPpriority > 0)
                    {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE [Priority]='" + nextUserPriority + "' AND WorkFlowTypeID='" + dmId + "' AND WFU.WorkFlowType='DM'");
                    string nextEmpName = "";
                    string nextUserId = "";
                    //string priorty = "0";
                    string nextTypeID = "";
                    ...
                    if (nextUserDetails.Rows.Count > 0)
                    {
                        ...
                        SQLQuery.ExecNonQry("UPDATE Dehumidifier SET CurrentWorkflowUser='" + nextEmpName + "' WHERE DehumidifierId = '" + dmId + "'");
                    }
                    string status = "";
                    string updateQuery = "";
                    if (nextUserDetails.Rows.Count == 0)
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update Dehumidifier SET  " + updateQuery + " WHERE DehumidifierId='" + dmId + "'");
                    BindWorkFlowUserGridView(dmId);
                    txtYourRemark.Text = "";
                    ReloadNotification();
                    PermissionToAction();
                }
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            try
            {
                ...
                if (status != "Hold")
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE  WorkFlowTypeID='" + lvId + "' AND WorkFlowType='DM'");
                    string empName = "";
                    string email = "";
                    string voucherNo = "";
                    ...
                    SQLQuery.ExecNonQry(" Update Dehumidifier SET WorkflowStatus='Hold',ReturnOrHoldUserID='" + wUserId + "',WorkflowApprovedDate= '" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss tt") + "' WHERE DehumidifierId='" + lvId + "'");
            ...
            if (status != "Return")
            {
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update Dehumidifier SET WorkflowStatus='Return', ReturnOrHoldUserID='" + wUserId + "' WHERE DehumidifierId='" + lvId + "'");
                    BindWorkFlowUserGridView(lvId);
                    txtYourRemark.Text = "";
                    PermissionToAction();
                    ReloadNotification();
    ...
    protected void btnDecline_OnClick(object sender, EventArgs e)
    {
        ...
        if (status != "Decline")
        {
            try
            {
                ...
                RunQuery.SQLQuery.ExecNonQry(" Update Dehumidifier SET WorkflowStatus='Decline', ReturnOrHoldUserID='" + wUserId + "' WHERE DehumidifierId='" + lvId + "'");
                BindWorkFlowUserGridView(lvId);
                txtYourRemark.Text = "";
                PermissionToAction();
                ReloadNotification();
