...
public partial class app_WorkFlowForSubStation : System.Web.UI.Page
{
    ...
    private void BindWorkFlowUserGridView(string id)
    {
        ...
        //string query = @"SELECT  Employee.Name AS EmployeeName ,WorkFlowUser.Priority, convert(varchar, WorkFlowUser.EsclationStartTime, 103) AS EsclationStartTime, convert(varchar, WorkFlowUser.EsclationEndTime, 103) AS EsclationEndTime,UserRemarks, ApproveDeclineDate, PermissionStatus FROM WorkFlowUser INNER JOIN Employee ON WorkFlowUser.EmployeeID = Employee.EmployeeID WHERE WorkFlowTypeID='" + ssId + "' Order By Priority DESC ";
        string query = @"SELECT Employee.Name + ', ' + Designation.Name AS EmployeeName,WorkflowUserSequence.SequenceBan + ' (' + CONVERT(varchar, WorkFlowUser.Priority) + ')' AS Priority, CONVERT(DATETIME,WorkFlowUser.EsclationStartTime, 121) AS EsclationStartTime, CONVERT(DATETIME, WorkFlowUser.EsclationEndTime, 121) AS EsclationEndTime, WorkFlowUser.UserRemarks, 
                  ...
                  Designation ON DesignationWithEmployee.DesignationID = Designation.DesignationID WHERE  (WorkFlowUser.WorkFlowTypeID = '" + id + "') AND (WorkFlowUser.WorkFlowType = 'SS') ORDER BY WorkFlowUser.Priority";
    ...
    {
        string query = @"SELECT S.SubStationID, S.SSVoucher, S.Date, S.PFI, S.Condition, S.Transformer, S.HTSwitchGear, S.HTBatteryChanger, 
                         S.HTAmpierXFormer, S.LTPanel, S.GeneratorBatteryCondition, S.LoadSheddingStartTime, S.LoadSheddingStopTime, S.GeneratorStartTime, 
                         S.GeneratorStopTime, S.FuelStockinLitre, S.FuelConsumptionInlitre, S.RunningHour, S.AttendentBy, S.Maintenance, 
                         E.Name AS ShiftInCharge, S.Remarks FROM SubStation AS S INNER JOIN
                         Employee AS E ON S.ShiftInCharge = E.EmployeeID WHERE SubStationID='" + id + "'";
        SqlCommand command = new SqlCommand(query, new SqlConnection(WebConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        command.Connection.Open();
        ...
        if (dataReader.Read())
        {
            ...
            txtSSNumber.Text = dataReader["SSVoucher"].ToString();
            txtPFI.Text = dataReader["PFI"].ToString();
            txtCondition.Text = dataReader["Condition"].ToString();
            txtTransformer.Text = dataReader["Transformer"].ToString();
            txtHTSwitchGear.Text = dataReader["HTSwitchGear"].ToString();
            txtHTBatteryChanger.Text = dataReader["HTBatteryChanger"].ToString();
            txtHTAmpierXFormer.Text = dataReader["HTAmpierXFormer"].ToString();
            txtLTPanel.Text = dataReader["LTPanel"].ToString();
            txtGeneratorBatteryCondition.Text = dataReader["GeneratorBatteryCondition"].ToString();
            txtLoadSheddingStartTime.Text = dataReader["LoadSheddingStartTime"].ToString();
            txtLoadSheddingStopTime.Text = dataReader["LoadSheddingStopTime"].ToString();
            txtGeneratorStartTime.Text = dataReader["GeneratorStartTime"].ToString();
            txtGeneratorStopTime.Text = dataReader["GeneratorStopTime"].ToString();
            txtFuelStockinLitre.Text = dataReader["FuelStockinLitre"].ToString();
            txtFuelConsumptionInlitre.Text = dataReader["FuelConsumptionInlitre"].ToString();
            txtRunningHour.Text = dataReader["RunningHour"].ToString();
            txtAttendentBy.Text = dataReader["AttendentBy"].ToString();
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            ...
            if (statusPermission != "Approved")
            {
                try
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE [Priority]='" + nextUserPriority + "' AND WorkFlowTypeID='" + esId + "' AND WFU.WorkFlowType='SS'");
                    string nextEmpName = "";
                    string nextUserId = "";
                    //string priorty = "0";
                    string nextTypeID = "";
                    ...
                    if (nextUserDetails.Rows.Count > 0)
                    {
                        ...
                        SQLQuery.ExecNonQry("UPDATE SubStation SET CurrentWorkflowUser='" + nextEmpName + "' WHERE SubStationID = '" + esId + "'");
                    }
                    string status = "";
                    string updateQuery = "";
                    if (nextUserDetails.Rows.Count == 0)
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update SubStation SET  " + updateQuery + " WHERE SubStationID='" + esId + "'");
    ...
    protected void btnHold_OnClick(object sender, EventArgs e)
    {
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            try
            {
                string id = Request.QueryString["Id"];
                string wUserId = EncryptDecrypt.DecryptString(id);
                string ssId = SQLQuery.ReturnString("SELECT WorkFlowTypeID From WorkFlowUser Where WorkFlowUserID='" + wUserId + "'");
                string status = SQLQuery.ReturnString("SELECT PermissionStatus From WorkFlowUser Where WorkFlowUserID='" + wUserId + "'");
                if (status != "Hold")
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE  WorkFlowTypeID='" + ssId + "' AND WorkFlowType='SS'");
                    string empName = "";
                    string email = "";
                    string voucherNo = "";
                    ...
                    SQLQuery.ExecNonQry(" Update SubStation SET WorkflowStatus='Hold',ReturnOrHoldUserID='" + wUserId + "',WorkflowApprovedDate= '" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss tt") + "' WHERE SubStationID='" + ssId + "'");
                    BindWorkFlowUserGridView(ssId);
                    txtYourRemark.Text = "";
                ...
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    if (userDetails.Rows.Count > 0)
                    {
                        ...
                        ssId = userDetails.Rows[0]["WorkFlowTypeID"].ToString();
                    ...
                    RunQuery.SQLQuery.ExecNonQry("Update SubStation SET WorkflowStatus='Return', ReturnOrHoldUserID='" + wUserId + "' WHERE SubStationID='" + ssId + "'");
                    BindWorkFlowUserGridView(ssId);
                    txtYourRemark.Text = "";
                    PermissionToAction();
                    ReloadNotification();
    ...
    protected void btnDecline_OnClick(object sender, EventArgs e)
    {
        ...
        if (status != "Decline")
        {
            try
            {
                ...
                RunQuery.SQLQuery.ExecNonQry("Update SubStation SET WorkflowStatus='Decline', ReturnOrHoldUserID='" + wUserId + "' WHERE SubStationID='" + ssId + "'");
                BindWorkFlowUserGridView(ssId);
                txtYourRemark.Text = "";
                PermissionToAction();
                ReloadNotification();
