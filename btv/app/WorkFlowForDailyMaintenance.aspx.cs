...
public partial class app_WorkFlowForDailyMaintenance : System.Web.UI.Page
{
...
FROM dbo.DailyMaintenanceDetails INNER JOIN dbo.Employee ON dbo.DailyMaintenanceDetails.Sign = dbo.Employee.EmployeeID WHERE dbo.DailyMaintenanceDetails.DailyMaintenanceID = '" + voucherID + "'";
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        //string query = @"SELECT  Employee.Name AS EmployeeName ,WorkFlowUser.Priority, convert(varchar, WorkFlowUser.EsclationStartTime, 103) AS EsclationStartTime, convert(varchar, WorkFlowUser.EsclationEndTime, 103) AS EsclationEndTime,UserRemarks, ApproveDeclineDate, PermissionStatus FROM WorkFlowUser INNER JOIN Employee ON WorkFlowUser.EmployeeID = Employee.EmployeeID WHERE WorkFlowTypeID='" + dmnId + "' Order By Priority DESC ";
        string query = @"SELECT Employee.Name + ', ' + Designation.Name AS EmployeeName,WorkflowUserSequence.SequenceBan + ' (' + CONVERT(varchar, WorkFlowUser.Priority) + ')' AS Priority, CONVERT(DATETIME,WorkFlowUser.EsclationStartTime, 121) AS EsclationStartTime, CONVERT(DATETIME, WorkFlowUser.EsclationEndTime, 121) AS EsclationEndTime, WorkFlowUser.UserRemarks, 
                  ...
                  Designation ON DesignationWithEmployee.DesignationID = Designation.DesignationID WHERE  (WorkFlowUser.WorkFlowTypeID = '" + id + "') AND (WorkFlowUser.WorkFlowType = 'DMN') ORDER BY WorkFlowUser.Priority";
        SqlCommand command = new SqlCommand(query, new SqlConnection(WebConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        command.Connection.Open();
        WorkFlowUserGridView.EmptyDataText = "No data added ...";
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            ...
            if (statusPermission != "Approved")
            {
                try
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE [Priority]='" + nextUserPriority + "' AND WorkFlowTypeID='" + esId + "' AND WFU.WorkFlowType='DMN'");
                    string nextEmpName = "";
                    string nextUserId = "";
                    //string priorty = "0";
                    string nextTypeID = "";
                    ...
                    if (nextUserDetails.Rows.Count > 0)
                    {
                        ...
                        SQLQuery.ExecNonQry("UPDATE DailyMaintenance SET CurrentWorkflowUser='" + nextEmpName + "' WHERE DailyMaintenanceID = '" + esId + "'");
                    }
                    string status = "";
                    string updateQuery = "";
                    if (nextUserDetails.Rows.Count == 0)
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update DailyMaintenance SET  " + updateQuery + " WHERE DailyMaintenanceID='" + esId + "'");
    ...
    protected void btnHold_OnClick(object sender, EventArgs e)
    {
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            try
            {
                string id = Request.QueryString["Id"];
                string wUserId = EncryptDecrypt.DecryptString(id);
                string dmnId = SQLQuery.ReturnString("SELECT WorkFlowTypeID From WorkFlowUser Where WorkFlowUserID='" + wUserId + "'");
                string status = SQLQuery.ReturnString("SELECT PermissionStatus From WorkFlowUser Where WorkFlowUserID='" + wUserId + "'");
                if (status != "Hold")
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE  WorkFlowTypeID='" + dmnId + "' AND WorkFlowType='DMN'");
                    string empName = "";
                    string email = "";
                    string voucherNo = "";
                    ...
                    SQLQuery.ExecNonQry(" Update DailyMaintenance SET WorkflowStatus='Hold',ReturnOrHoldUserID='" + wUserId + "',WorkflowApprovedDate= '" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss tt") + "' WHERE DailyMaintenanceID='" + dmnId + "'");
                    BindWorkFlowUserGridView(dmnId);
                    txtYourRemark.Text = "";
                ...
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    if (userDetails.Rows.Count > 0)
                    {
                        ...
                        dmnId = userDetails.Rows[0]["WorkFlowTypeID"].ToString();
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update DailyMaintenance SET WorkflowStatus='Return', ReturnOrHoldUserID='" + wUserId + "' WHERE DailyMaintenanceID='" + dmnId + "'");
                    BindWorkFlowUserGridView(dmnId);
                    txtYourRemark.Text = "";
                    PermissionToAction();
                    ReloadNotification();
    ...
    protected void btnDecline_OnClick(object sender, EventArgs e)
    {
        ...
        if (status != "Decline")
        {
            try
            {
                ...
                RunQuery.SQLQuery.ExecNonQry("Update DailyMaintenance SET WorkflowStatus='Decline', ReturnOrHoldUserID='" + wUserId + "' WHERE DailyMaintenanceID='" + dmnId + "'");
                BindWorkFlowUserGridView(dmnId);
                txtYourRemark.Text = "";
                PermissionToAction();
                ReloadNotification();
