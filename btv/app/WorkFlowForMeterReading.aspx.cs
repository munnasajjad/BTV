...
public partial class app_WorkFlowForMeterReading : System.Web.UI.Page
{
    ...
    private void BindWorkFlowUserGridView(string id)
    {
        ...
        string query = @"SELECT Employee.Name + ', ' + Designation.Name AS EmployeeName,WorkflowUserSequence.SequenceBan + ' (' + CONVERT(varchar, WorkFlowUser.Priority) + ')' AS Priority, CONVERT(DATETIME,WorkFlowUser.EsclationStartTime, 121) AS EsclationStartTime, CONVERT(DATETIME, WorkFlowUser.EsclationEndTime, 121) AS EsclationEndTime, WorkFlowUser.UserRemarks, 
                  ...
                  Designation ON DesignationWithEmployee.DesignationID = Designation.DesignationID WHERE  (WorkFlowUser.WorkFlowTypeID = '" + id + "') AND (WorkFlowUser.WorkFlowType = 'MR') ORDER BY WorkFlowUser.Priority";
    ...
    {
        string query = @"SELECT dbo.MeterReading.MeterReadingId, dbo.MeterReading.MRVoucher, dbo.MeterReading.MainOfficeId, dbo.Location.Name AS MainOffice, dbo.MeterReading.Date, dbo.MeterReading.SuppliedBy, dbo.MeterReading.MeterNumber, 
                        dbo.MeterReading.ConsumptionTime, dbo.MeterReading.KVARH, dbo.MeterReading.MaximumDemand, dbo.MeterReading.UHICondition, dbo.MeterReading.[1C-Peak], dbo.MeterReading.[2C-Peak], 
                        dbo.MeterReading.ReadingTakenBy, dbo.MeterReading.ShiftInCharge, dbo.Employee.Name AS ShiftInChargeName, dbo.MeterReading.Remarks
                        FROM dbo.Location INNER JOIN dbo.MeterReading ON dbo.Location.LocationID = dbo.MeterReading.MainOfficeId INNER JOIN
                        dbo.Employee ON dbo.MeterReading.ShiftInCharge = dbo.Employee.EmployeeID Where MeterReadingId='" + id + "'";
        SqlCommand command = new SqlCommand(query, new SqlConnection(WebConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        command.Connection.Open();
        ...
        if (dataReader.Read())
        {
            ...
            txtMRNumber.Text = dataReader["MRVoucher"].ToString();
            txtSuppliedBy.Text = dataReader["SuppliedBy"].ToString();
            txtMeterNumber.Text = dataReader["MeterNumber"].ToString();
            txtConsumptionTime.Text = dataReader["ConsumptionTime"].ToString();
            txtKVARH.Text = dataReader["KVARH"].ToString();
            txtMaximumDemand.Text = dataReader["MaximumDemand"].ToString();
            txtUHICondition.Text = dataReader["UHICondition"].ToString();
            txt1CPeak.Text = dataReader["1C-Peak"].ToString();
            txt2CPeak.Text = dataReader["2C-Peak"].ToString();
            txtReadingTakenBy.Text = dataReader["ReadingTakenBy"].ToString();
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            ...
            if (statusPermission != "Approved")
            {
                try
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE [Priority]='" + nextUserPriority + "' AND WorkFlowTypeID='" + esId + "' AND WFU.WorkFlowType='MR'");
                    string nextEmpName = "";
                    string nextUserId = "";
                    //string priorty = "0";
                    string nextTypeID = "";
                    ...
                    if (nextUserDetails.Rows.Count > 0)
                    {
                        ...
                        SQLQuery.ExecNonQry("UPDATE MeterReading SET CurrentWorkflowUser='" + nextEmpName + "' WHERE MeterReadingId = '" + esId + "'");
                    }
                    string status = "";
                    string updateQuery = "";
                    if (nextUserDetails.Rows.Count == 0)
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update MeterReading SET  " + updateQuery + " WHERE MeterReadingId='" + esId + "'");
        ...
        if (WorkFlowUserGridView.Rows.Count > 0)
        {
            try
            {
                ...
                if (status != "Hold")
                {
                  ...
                  Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE  WorkFlowTypeID='" + lvId + "' AND WorkFlowType='MR'");
                    string empName = "";
                    string email = "";
                    string voucherNo = "";
                    ...
                    SQLQuery.ExecNonQry(" Update MeterReading SET WorkflowStatus='Hold',ReturnOrHoldUserID='" + wUserId + "',WorkflowApprovedDate= '" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss tt") + "' WHERE MeterReadingId='" + lvId + "'");
            ...
            if (status != "Return")
            {
                try
                {
                  ...
                  FROM WorkFlowUser AS WFU INNER JOIN DesignationWithEmployee AS DWE ON WFU.EmployeeID = DWE.Id INNER JOIN Employee ON DWE.EmployeeID = Employee.EmployeeID WHERE WorkFlowUserID='" + wUserId + "'");
                    ...
                    RunQuery.SQLQuery.ExecNonQry(" Update MeterReading SET WorkflowStatus='Return', ReturnOrHoldUserID='" + wUserId + "' WHERE MeterReadingId='" + lvId + "'");
                    BindWorkFlowUserGridView(lvId);
                    txtYourRemark.Text = "";
                    PermissionToAction();
                    ReloadNotification();
    ...
    protected void btnDecline_OnClick(object sender, EventArgs e)
    {
        ...
        if (status != "Decline")
        {
            try
            {
                ...
                RunQuery.SQLQuery.ExecNonQry(" Update MeterReading SET WorkflowStatus='Decline', ReturnOrHoldUserID='" + wUserId + "' WHERE MeterReadingId='" + lvId + "'");
                BindWorkFlowUserGridView(lvId);
                txtYourRemark.Text = "";
                PermissionToAction();
                ReloadNotification();
