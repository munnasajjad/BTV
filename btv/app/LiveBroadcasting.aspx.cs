...
public partial class app_LiveBroadcasting : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            txtLBNumber.Text = LogMonitorGenerateVoucher.GetLBVoucherNumber(Convert.ToDateTime(txtDate.Text), User.Identity.Name, SQLQuery.GetDepartmentSectionId(User.Identity.Name));
            BindDesignation();
            BindEmployee();
            BindddPriority();
    ...
    //            {
    //                RunQuery.SQLQuery.ExecNonQry(" INSERT INTO LiveBroadcasting (SerialNo, Date, Venue, ProposedStartingTime, StaringTime, EndingTime, Source, ActiveSource, StandBySource, ResponsiblePerson, ContactNumber, ShiftInCharge, Remarks,EntryBy) VALUES ('" + txtSerialNo.Text.Replace("'", "''") + "', '" + Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd") + "', '" + txtVenue.Text.Replace("'", "''") + "', '" + txtProposedStartingTime.Text.Replace("'", "''") + "', '" + txtStaringTime.Text.Replace("'", "''") + "', '" + txtEndingTime.Text.Replace("'", "''") + "', '" + txtSource.Text.Replace("'", "''") + "', '" + ddlActiveSource.SelectedValue + "', '" + ddlStandBySource.SelectedValue+ "', '" + txtResponsiblePerson.Text.Replace("'", "''") + "', '" + txtContactNumber.Text.Replace("'", "''") + "', '" + txtShiftInCharge.Text.Replace("'", "''") + "', '" + txtRemarks.Text.Replace("'", "''") + "','"+lName+"')    ");
    //                ClearControls();
    ...
    //            {
    //                RunQuery.SQLQuery.ExecNonQry(" Update  LiveBroadcasting SET SerialNo= '" + txtSerialNo.Text.Replace("'", "''") + "',  Date= '" + Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd") + "',  Venue= '" + txtVenue.Text.Replace("'", "''") + "',  ProposedStartingTime= '" + txtProposedStartingTime.Text.Replace("'", "''") + "',  StaringTime= '" + txtStaringTime.Text.Replace("'", "''") + "',  EndingTime= '" + txtEndingTime.Text.Replace("'", "''") + "',  Source= '" + txtSource.Text.Replace("'", "''") + "',  ActiveSource= '" + ddlActiveSource.SelectedValue + "',  StandBySource= '" + ddlStandBySource.SelectedValue + "',  ResponsiblePerson= '" + txtResponsiblePerson.Text.Replace("'", "''") + "',  ContactNumber= '" + txtContactNumber.Text.Replace("'", "''") + "',  ShiftInCharge= '" + txtShiftInCharge.Text.Replace("'", "''") + "',  Remarks= '" + txtRemarks.Text.Replace("'", "''") + "' WHERE LiveBroadcastingId='" + lblId.Text + "' ");
    //                ClearControls();
    //                btnSave.Text = "Save";
    ...
    //            DataTable dt = SQLQuery.ReturnDataTable(" SELECT LiveBroadcastingId, SerialNo,Date,Venue,ProposedStartingTime,StaringTime,EndingTime,Source,ActiveSource,StandBySource,ResponsiblePerson,ContactNumber,ShiftInCharge,Remarks FROM LiveBroadcasting WHERE LiveBroadcastingId='" + lblId.Text + "'");
    ...
    //                txtVenue.Text = dtx["Venue"].ToString();
    //                txtProposedStartingTime.Text = dtx["ProposedStartingTime"].ToString();
    //                txtStaringTime.Text = dtx["StaringTime"].ToString();
    //                txtEndingTime.Text = dtx["EndingTime"].ToString();
    //                txtSource.Text = dtx["Source"].ToString();
    //                ddlActiveSource.SelectedValue = dtx["ActiveSource"].ToString();
    //                ddlStandBySource.SelectedValue= dtx["StandBySource"].ToString();
    //                txtResponsiblePerson.Text = dtx["ResponsiblePerson"].ToString();
    //                txtContactNumber.Text = dtx["ContactNumber"].ToString();
    ...
    protected void GridView1_OnRowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        if (SQLQuery.OparatePermission(lName, "Delete") == "1")
        {
            ...
            RunQuery.SQLQuery.ExecNonQry(" Delete LiveBroadcasting WHERE LiveBroadcastingId='" + lblId.Text + "' ");
            BindGrid();
    ...
    //{
    //    DataTable dt = SQLQuery.ReturnDataTable(@"SELECT  LiveBroadcastingId, SerialNo,  CONVERT(varchar,Date, 103) AS Date, Venue, ProposedStartingTime, StaringTime, EndingTime, Source, ActiveSource, StandBySource, ResponsiblePerson, ContactNumber, ShiftInCharge, Remarks, EntryBy FROM LiveBroadcasting");
    ...
    private void ClearControls()
    {
        ...
        txtDate.Text = "";
        txtVenue.Text = "";
        txtProposedStartingTime.Text = "";
        txtStaringTime.Text = "";
        txtEndingTime.Text = "";
        txtSource.Text = "";
        //txtActiveSource.Text = "";
        //txtStandBySource.Text = "";
    ...
    protected void GridView1_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            if (SQLQuery.OparatePermission(lName, "Update") == "1")
            {
                ...
                Label labelLBVoucher = GridView1.Rows[index].FindControl("lblLBNumber") as Label;
                string saveMode = SQLQuery.ReturnString(@"SELECT SaveMode FROM LiveBroadcasting WHERE LiveBroadcastingId='" + lblEditId.Text + "'");
                string workflowStatus = SQLQuery.ReturnString(@"SELECT WorkflowStatus FROM LiveBroadcasting WHERE LiveBroadcastingId='" + lblEditId.Text + "'");
                if (Page.User.IsInRole("Super Admin"))
                {
                ...
                else if (Page.User.Identity.Name == lblEntryBy.Text)
                {
                    ...
                    {
                        Notify("This " + labelLBVoucher.Text + " already submitted. If you need to any change please contact higher authority.", "warn", lblMsg);
                    }
                }
                else
                {
                    Notify("This number" + labelLBVoucher.Text + " entry by user is " + lblEntryBy.Text + ". You are not authorize edit the voucher. If you need to any change please contact higher authority.", "warn", lblMsg);
                }
            }
            else
            {
    ...
    private void BindGrid()
    {
        ...
        string sql = @"SELECT LB.LiveBroadcastingId, LB.LBVoucher, LB.MainOfficeId, LB.FunctionalOfficeId, LB.DepartmentId, LB.FinYear, LB.SerialNo, LB.Date, LB.Venue, LB.ProposedStartingTime, LB.StaringTime, LB.EndingTime, LB.Source, LB.ActiveSource, LB.StandBySource, LB.ResponsiblePerson, LB.ContactNumber, E.Name AS ShiftInCharge, LB.PreparedBy, LB.PreparedDate, LB.Checkerby, LB.CheckerDate, LB.Approvedby, LB.ApprovedDate, LB.SaveMode, LB.SubmitDate, LB.WorkflowStatus, LB.WorkflowApprovedDate, LB.CurrentWorkflowUser, LB.ReturnOrHoldUserID, LB.Remarks, LB.EntryBy FROM LiveBroadcasting AS LB INNER JOIN Employee AS E ON LB.ShiftInCharge = E.EmployeeID " + query + "";
    ...
    private bool PriorityCheck()
    {
        ...
        DataTable priorityDataTable = SQLQuery.ReturnDataTable(@"SELECT Priority,EmployeeID FROM WorkFlowUser WHERE WorkFlowTypeID = '" + hiddenLBID.Value + "' AND EntryBy='" + lName + "' AND WorkFlowType = 'LB'");

        foreach (DataRow priorityDataRow in priorityDataTable.Rows)
        {
            string escDate = SQLQuery.ReturnString("SELECT IsNull(Count(Priority),0) FROM WorkFlowUser WHERE WorkFlowTypeID = '" + hiddenLBID.Value + "' AND WorkFlowType = 'LB'");
    ...
    private void InsertToWorkFlowUser()
    {
        ...
        {
            typeId = Convert.ToInt32(hiddenLBID.Value);
        }
        ...
        command = new SqlCommand(@"INSERT INTO WorkFlowUser ( WorkFlowTypeID,WorkFlowType, EmployeeID,DesignationId, Priority, EsclationDay,  Remark, TaskStatus, EntryBy, EntryDate) 
                                       VALUES (@WorkFlowTypeID,'LB',@EmployeeID,@DesignationId,@Priority,@EsclationDay,@Remark,'1',@EntryBy,@EntryDate )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    private void BindWorkFlowUserGridView()
    {
        ...
        query = @"SELECT WorkFlowUser.WorkFlowUserID, WorkFlowUser.Priority, WorkFlowUser.EsclationDay, WorkflowUserSequence.SequenceBan + ' (' + CONVERT(varchar, WorkFlowUser.Priority) + ')' AS SequenceBan, 
                  ...
                  Designation ON DesignationWithEmployee.DesignationID = Designation.DesignationID WHERE WorkFlowTypeID='" + hiddenLBID.Value + "' AND WorkFlowUser.WorkFlowType = 'LB' AND EntryBy = '" + lName + "' Order By Priority ASC";
    ...
    {
        DataTable dt = SQLQuery.ReturnDataTable("SELECT WorkFlowUserID,EsclationDay,EsclationEndTime FROM WorkFlowUser WHERE WorkFlowTypeID='" + hiddenLBID.Value + "' AND WorkFlowType = 'LB' AND EntryBy = '" + User.Identity.Name + "'");
    ...
    private bool PriorityCheckForUpdate()
    {
        string lName = Page.User.Identity.Name.ToString();
        bool priorityStatus = true;
        DataTable priorityDataTable = SQLQuery.ReturnDataTable(@"SELECT Priority,EmployeeID FROM WorkFlowUser WHERE WorkFlowTypeID = '" + hiddenLBID.Value + "' AND WorkFlowType = 'LB'");
        foreach (DataRow priorityDataRow in priorityDataTable.Rows)
        {
            if (ddEmployee.SelectedValue == priorityDataRow["EmployeeID"].ToString())
            {
                string priority = SQLQuery.ReturnString("SELECT IsNull(Count(Priority),0) FROM WorkFlowUser WHERE WorkFlowTypeID = '" + hiddenLBID.Value + "' AND Priority='" + ddlPriority.SelectedValue + "' AND EmployeeID <>'" + priorityDataRow["EmployeeID"] + "' AND WorkFlowType = 'LB'");
    ...
    {
        SQLQuery.PopulateDropDown("SELECT SequenceId, SequenceBan +' ('+Convert(varchar,Priority)+')' AS SequenceBan, SequenceEng, Priority, Type FROM WorkflowUserSequence WHERE  (Type = 'LB')", ddlPriority, "Priority", "SequenceBan");
    }
    private void BindActiveSource()
    {
        SQLQuery.PopulateDropDown("SELECT Id, LiveBroadcastingSourceName FROM LiveBroadcastingSource", ddlActiveSource, "Id", "LiveBroadcastingSourceName");
    }
    private void BindStandBySource()
    {
        SQLQuery.PopulateDropDown("SELECT Id, LiveBroadcastingSourceName FROM LiveBroadcastingSource", ddlStandBySource, "Id", "LiveBroadcastingSourceName");
    }
    protected void btnWorkFlowSave_OnClick(object sender, EventArgs e)
    {
        ...
        {
            string lName = Page.User.Identity.Name;
            string isUserExists = SQLQuery.ReturnString("SELECT EmployeeID FROM WorkFlowUser WHERE DesignationId='" + ddlDesignation.SelectedValue + "' AND EmployeeID = '" + ddEmployee.SelectedValue + "'AND WorkFlowTypeID ='" + hiddenLBID.Value + "' AND WorkFlowType = 'LB' AND EntryBy = '" + lName + "'");
    ...
    private void NotifyToEmployee(string employeeID, string lvNumber, string mnId)
    {
        ...
        foreach (DataRow item in dt.Rows)
        {
            ...
            SQLQuery.ExecNonQry("UPDATE LiveBroadcasting SET CurrentWorkflowUser='" + name + "' WHERE LiveBroadcastingId = '" + mnId + "'");
    ...
    private void SaveData(string saveMode)
    {
        ...
        //(" INSERT INTO LiveBroadcasting (Date, SuppliedBy, MeterNumber, ConsumptionTime, KVARH, MaximumDemand, UHICondition, [1C-Peak], [2C-Peak], ReadingTakenBy, Remarks,EntryBy) VALUES ('" + Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd") + "', '" + ddlSupplied.SelectedValue + "', '" + txtMeterNumber.Text.Replace("'", "''") + "', '" + ddlConsumptionTime.SelectedValue + "', '" + txtKVARH.Text.Replace("'", "''") + "', '" + txtMaximumDemand.Text.Replace("'", "''") + "', '" + txtUHICondition.Text.Replace("'", "''") + "', '" + txt1CPeak.Text.Replace("'", "''") + "', '" + txt2CPeak.Text.Replace("'", "''") + "', '" + txtReadingTakenBy.Text.Replace("'", "''") + "', '" + txtRemarks.Text.Replace("'", "''") + "','" + lName + "')");
        SQLQuery.ExecNonQry("INSERT INTO LiveBroadcasting (" + sqlColumn + "SaveMode, LBVoucher, MainOfficeId, FunctionalOfficeId, DepartmentId, FinYear, SerialNo, Date, Venue, ProposedStartingTime, StaringTime, EndingTime, Source, ActiveSource, StandBySource, ResponsiblePerson, ContactNumber, ShiftInCharge, Remarks, PreparedBy, EntryBy) VALUES (" + sqlValue + "'" + saveMode + "', '" + txtLBNumber.Text + "','" + SQLQuery.GetLocationID(User.Identity.Name) + "','" + SQLQuery.GetCenterId(User.Identity.Name) + "','" + SQLQuery.GetDepartmentSectionId(User.Identity.Name) + "','" + GenerateVoucherNumber.GetFinYear(Convert.ToDateTime(txtDate.Text)) + "','" + txtSerialNo.Text.Replace("'", "''") + "', '" + Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd") + "', '" + txtVenue.Text.Replace("'", "''") + "', '" + txtProposedStartingTime.Text.Replace("'", "''") + "', '" + txtStaringTime.Text.Replace("'", "''") + "', '" + txtEndingTime.Text.Replace("'", "''") + "', '" + txtSource.Text.Replace("'", "''") + "', '" + ddlActiveSource.SelectedValue + "', '" + ddlStandBySource.SelectedValue + "', '" + txtResponsiblePerson.Text.Replace("'", "''") + "', '" + txtContactNumber.Text.Replace("'", "''") + "', '" + ddShiftInCharge.SelectedValue + "', '" + txtRemarks.Text.Replace("'", "''") + "','" + SQLQuery.GetEmployeeID(User.Identity.Name) + "','" + User.Identity.Name + "')");
        string mnId = SQLQuery.ReturnString("SELECT MAX(LiveBroadcastingId) AS mnId FROM LiveBroadcasting WHERE EntryBy='" + lName + "'");
        SQLQuery.ExecNonQry("UPDATE WorkFlowUser SET WorkFlowTypeID='" + mnId + "',VoucherNo='" + txtLBNumber.Text + "'  WHERE WorkFlowTypeID = '0' AND WorkFlowType='LB' AND EntryBy='" + lName + "' ");
        ...
        if (saveMode == "Submitted")
        {
            string sqlquery = @"SELECT WorkFlowUserID, WorkFlowTypeID, WorkFlowType, VoucherNo, EmployeeID, DesignationId, Priority, EsclationStartTime, EsclationEndTime, EsclationDay, Remark, TaskStatus, UserRemarks, ApproveDeclineDate, 
                            PermissionStatus, EntryBy, EntryDate, IsActive FROM WorkFlowUser WHERE WorkFlowTypeID='" + mnId + "' AND WorkFlowType='LB'";
    ...
    {
        string returnUser = "";
        string workflowStatus = SQLQuery.ReturnString("SELECT WorkflowStatus FROM LiveBroadcasting WHERE LiveBroadcastingId='" + hiddenLBID.Value + "'");
        if (workflowStatus == "Return")
        {
            returnUser = SQLQuery.ReturnString("SELECT ReturnOrHoldUserID FROM LiveBroadcasting WHERE LiveBroadcastingId='" + hiddenLBID.Value + "'");
        ...
        RunQuery.SQLQuery.ExecNonQry("UPDATE LiveBroadcasting SET SerialNo= '" + txtSerialNo.Text.Replace("'", "''") + "',  Date= '" + Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd") + "',  Venue= '" + txtVenue.Text.Replace("'", "''") + "',  ProposedStartingTime= '" + txtProposedStartingTime.Text.Replace("'", "''") + "',  StaringTime= '" + txtStaringTime.Text.Replace("'", "''") + "',  EndingTime= '" + txtEndingTime.Text.Replace("'", "''") + "',  Source= '" + txtSource.Text.Replace("'", "''") + "',  ActiveSource= '" + ddlActiveSource.SelectedValue + "',  StandBySource= '" + ddlStandBySource.SelectedValue + "',  ResponsiblePerson= '" + txtResponsiblePerson.Text.Replace("'", "''") + "',  ContactNumber= '" + txtContactNumber.Text.Replace("'", "''") + "',  ShiftInCharge= '" + ddShiftInCharge.SelectedValue + "',  Remarks= '" + txtRemarks.Text.Replace("'", "''") + "', WorkflowStatus='" + workflowStatus + "',SaveMode='" + saveMode + "'" + sqlColumn + " WHERE LiveBroadcastingId='" + lblId.Text + "' ");
        if (saveMode == "Submitted")
        {
            if (returnUser != "")
            {
                ...
                foreach (DataRow item in dtUser.Rows)
                {
                    ...
                    NotifyToEmployee(item["EmployeeID"].ToString(), item["VoucherNo"].ToString(), hiddenLBID.Value);
                }
            }
            else
            {
                string sqlquery = @"SELECT WorkFlowUserID, WorkFlowTypeID, WorkFlowType, VoucherNo, EmployeeID, DesignationId, Priority, EsclationStartTime, EsclationEndTime, EsclationDay, Remark, TaskStatus, UserRemarks, ApproveDeclineDate, 
                  PermissionStatus, EntryBy, EntryDate, IsActive FROM WorkFlowUser WHERE WorkFlowTypeID='" + hiddenLBID.Value + "' AND WorkFlowType='LB'";
        ...
        try
        {
            ...
            if (btnDraft.Text.ToUpper() == "SAVE AS DRAFT")
            {
                if (SQLQuery.OparatePermission(lName, "Insert") == "1")
                {
                    ...
                    BindGrid();
                    BindWorkFlowUserGridView();
                    txtLBNumber.Text = LogMonitorGenerateVoucher.GetLBVoucherNumber(Convert.ToDateTime(txtDate.Text),
